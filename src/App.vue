<template>
  ...
  <TheModal :showModal="showPromoForm" @mousedown="showPromoForm = false">
    <PromoForm @submit="createPromo" :formVals="defaultForm" />
  </TheModal>
  <TheModal :showModal="showEditPromoForm" @mousedown="showEditPromoForm = false" >
    <PromoForm @submit="updatePromo" :formVals="curPromo"><button>Update</button></PromoForm>
  </TheModal>

  <TheModal :showModal="showSettings" @mousedown="showSettings = false" >
    <TheSettings 
  ...
  <ul v-if="sets.length > 0">
    ...
    <li class="flex gap" v-for="(promo,i) in sets[currentSet].promos" :id="promo.id" >
...
function start(idx, auto_break_flag = true, auto_promo_flag = true){
  ...
  runTimer(dur, () => {
    ...
    start(idx, autoplay.value.autoPlayBreak, autoplay.value.autoPlayPromo);
  });
